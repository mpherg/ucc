...
*/
size_t CUtil::FindKeyword(const string &str, const string &keyword, size_t start, size_t end, bool casesensitive)
{
...
string CUtil::ExtractFilename(const string &filepath)
{
	...
	if (idx == string::npos)
		idx = filepath.find_last_of("/");	// Unix
	if (idx != string::npos)
...
string CUtil::ConvertClearCaseFile(const string &fileName)
{
	...
	size_t loc = fileName1.find_last_of("@@");
	if (loc == string::npos)
		return fileName1;
	else
...
*/
void CUtil::AddError(const string &err, const string &outDir)
{
	static ofstream errFile;
	static string filePath;

	// set the error file path if empty
	if (filePath.empty())
	{
		time_t myTime;
		struct tm *myLocalTime;
