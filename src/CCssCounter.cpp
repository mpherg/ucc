...
*/
void CCssCounter::LSLOC(results* result, string &line, string &strLSLOC,
						char &lastLinesLastChar, unsigned int &phys_exec_lines, unsigned int &phys_data_lines)
{
	...
	while (index < tmp.length())
	{
		...
		case ';': case '}':
			...
			if (result->addSLOC(strLSLOC, trunc_flag))
